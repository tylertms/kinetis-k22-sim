...
static bool write_ftm_register(K22Timing* timing, uint8_t instance, uint32_t offset, uint8_t size,
    ...
    if (offset == 0) {
        ...
        if (offset == 0x54u || offset == 0x7cu || offset == 0x88u) {
            ...
            const uint8_t enabled = k22_timing_internal_ftm_fault_mode(ftm) == 0u
                                        ? 0u
...
static void rtc_software_reset(K22Timing* timing) {
    ...
    timing->rtc_cr = 1u;
    timing->rtc_sr = 1u;
    timing->rtc_lr = k22_timing_internal_rtc_access_reset(timing);
    timing->rtc_ier = 7u;
    ...
    if (address >= LPTMR_BASE && address < LPTMR_BASE + 0x10u && size == 4 &&
        ...
        case 0: {
            ...
            if ((timing->lptmr_csr & 1u) == 0) {
                ...
                timing->lptmr_observed_active = k22_timing_internal_lptmr_selected_active(timing);
                timing->lptmr_filter_ticks = 0u;
            }
            k22_timing_internal_set_irq(timing, IRQ_LPTMR, (timing->lptmr_csr & 0xc0u) == 0xc0u);
            return true;
        }
        case 4:
            if ((timing->lptmr_csr & 1u) == 0)
...
void k22_timing_reset(K22Timing* timing, uint8_t srs0, uint8_t srs1) {
    ...
    timing->rtc_lr = k22_timing_internal_rtc_access_reset(timing);
    timing->rtc_ier = 7u;
    timing->rtc_war = k22_timing_internal_rtc_access_reset(timing);
    ...
    k22_timing_internal_update_rtc_irq(timing);
    k22_timing_internal_set_irq(timing, IRQ_RTC_SECONDS, false);
    k22_timing_internal_set_irq(timing, IRQ_WDOG_EWM, false);
}
...
void k22_timing_advance(K22Timing* timing, uint32_t core_cycles) {
    ...
    k22_timing_internal_advance_wdog(timing, core_cycles);
    k22_timing_internal_advance_ewm(timing, core_cycles);
    k22_timing_internal_advance_pit(timing, core_cycles);
...
bool k22_timing_trigger_low_voltage_warning(K22Timing* timing) {
    if (timing == NULL || timing->profile == NULL ||
        !k22_timing_internal_has(timing, K22_PERIPHERAL_PMC))
        return false;
...
bool k22_timing_trigger_low_voltage_detect(K22Timing* timing) {
    ...
    if ((timing->pmc[0] & 0x10u) != 0u)
        k22_timing_internal_signal_reset(timing, 2u, 0u);
    else
        k22_timing_internal_update_pmc_irq(timing);
    return true;
}
...
static void llwu_wake(K22Timing* timing) {
    if (timing->smc[3] == 0x40u)
        k22_timing_internal_signal_reset(timing, 1u, 0u);
    else
        k22_timing_internal_update_llwu_irq(timing);
}
...
bool k22_timing_set_llwu_pin(K22Timing* timing, uint8_t pin, bool high) {
    if (timing == NULL || timing->profile == NULL || pin >= 16u ||
        !k22_timing_internal_has(timing, K22_PERIPHERAL_LLWU))
        return false;
...
bool k22_timing_ewm_output(const K22Timing* timing) {
    return timing != NULL && timing->profile != NULL &&
           k22_timing_internal_has(timing, K22_PERIPHERAL_EWM) && timing->ewm_output;
}
