...
static bool timing_peripheral(KinetisPeripheralId id) {
    return id == KINETIS_PERIPHERAL_FTM0 || id == KINETIS_PERIPHERAL_FTM1 ||
           ...
           id == KINETIS_PERIPHERAL_PDB0 || id == KINETIS_PERIPHERAL_PIT ||
           id == KINETIS_PERIPHERAL_RTC || id == KINETIS_PERIPHERAL_LPTMR0 ||
           id == KINETIS_PERIPHERAL_SIM || id == KINETIS_PERIPHERAL_WDOG ||
           id == KINETIS_PERIPHERAL_EWM || id == KINETIS_PERIPHERAL_MCG ||
           id == KINETIS_PERIPHERAL_OSC || id == KINETIS_PERIPHERAL_LLWU ||
           id == KINETIS_PERIPHERAL_PMC || id == KINETIS_PERIPHERAL_SMC ||
           id == KINETIS_PERIPHERAL_RCM;
}
...
static bool io_peripheral(KinetisPeripheralId id) {
    return id == KINETIS_PERIPHERAL_FB || id == KINETIS_PERIPHERAL_SYSMPU ||
           ...
           id == KINETIS_PERIPHERAL_GPIOE || id == KINETIS_PERIPHERAL_MCM;
}

static uint8_t data_irq(KinetisDataInterrupt interrupt) {
...
void kinetis_internal_timing_trigger(void* context, KinetisTimingTrigger type, uint8_t instance,
    ...
    if (type == KINETIS_TIMING_TRIGGER_PDB_ADC) {
        ...
        kinetis_data_dac_trigger(device->data, instance);
    } else {
...
bool kinetis_internal_peripheral_clock_enabled(const Kinetis* device, KinetisPeripheralId id) {
    ...
    if (id >= KINETIS_PERIPHERAL_GPIOA && id <= KINETIS_PERIPHERAL_GPIOE) {
        return gate(scgc5, (uint8_t)(9u + id - KINETIS_PERIPHERAL_GPIOA));
    }
    switch (id) {
...
void kinetis_internal_refresh_serial_signals(Kinetis* device) {
    ...
    for (uint8_t dma_index = 0; dma_index < KINETIS_SERIAL_DMA_COUNT; dma_index++) {
        if (dma_sources[dma_index] != UINT8_MAX &&
            ...
            kinetis_data_dma_request(device->data, dma_sources[dma_index]);
        }
    }
}
