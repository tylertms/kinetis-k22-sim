    ...
    *output_value = kinetis_internal_raw_load(device, address, 1u);
    if (address == K22_CMT + 5u && (*output_value & 0x80u) != 0u)
        device->cmt_eoc_read = true;
             ...
             (kinetis_internal_raw_load(device, K22_CMT + 0x0bu, 1u) & 1u) == 0u)
...
static void cmt_refresh_irq(Kinetis* device) {
    ...
    const uint8_t dma_control = (uint8_t)kinetis_internal_raw_load(device, K22_CMT + 0x0bu, 1u);
    if (device->cpu != NULL)
...
static void cmt_raise_cycle(Kinetis* device, uint8_t cmt_control) {
    kinetis_internal_raw_store(device, K22_CMT + 5u, 1u, cmt_control | 0x80u);
...
static uint64_t cmt_clock_ticks(const Kinetis* device) {
    const uint8_t cmt_control = (uint8_t)kinetis_internal_raw_load(device, K22_CMT + 5u, 1u);
    const uint64_t clock_divider = kinetis_internal_raw_load(device, K22_CMT + 0x0au, 1u) + 1u;
...
static void cmt_load_cycle(Kinetis* device) {
    ...
    const uint64_t mark_count = (kinetis_internal_raw_load(device, K22_CMT + 6u, 1u) << 8u) |
                                kinetis_internal_raw_load(device, K22_CMT + 7u, 1u);
    const uint64_t space_count = (kinetis_internal_raw_load(device, K22_CMT + 8u, 1u) << 8u) |
                                 kinetis_internal_raw_load(device, K22_CMT + 9u, 1u);
    ...
    const uint64_t carrier_high_count =
        kinetis_internal_raw_load(device, K22_CMT + carrier_offset, 1u);
    const uint64_t carrier_low_count =
        kinetis_internal_raw_load(device, K22_CMT + carrier_offset + 1u, 1u);
...
static void cmt_start(Kinetis* device, uint8_t cmt_control) {
    ...
    const uint64_t clock_divider = kinetis_internal_raw_load(device, K22_CMT + 0x0au, 1u);
...
static void cmt_clear_eoc(Kinetis* device) {
    kinetis_internal_raw_store(device, K22_CMT + 5u, 1u,
                               kinetis_internal_raw_load(device, K22_CMT + 5u, 1u) & 0x7fu);
...
void kinetis_internal_cmt_advance(Kinetis* device, uint32_t cycle_count) {
    ...
    while (elapsed_ticks != 0u && device->cmt_running) {
        ...
        if (device->cmt_stop_pending ||
            (kinetis_internal_raw_load(device, K22_CMT + 5u, 1u) & 1u) == 0u) {
...
K22PeripheralId kinetis_internal_serial_endpoint_peripheral(KinetisSerialEndpoint endpoint) {
    static const K22PeripheralId peripherals[KINETIS_SERIAL_ENDPOINT_COUNT] = {
    ...
    return endpoint < KINETIS_SERIAL_ENDPOINT_COUNT ? peripherals[endpoint] : K22_PERIPHERAL_COUNT;
}

bool kinetis_internal_serial_endpoint_available(const Kinetis* device,
                                                KinetisSerialEndpoint endpoint) {
    const K22PeripheralId peripheral = kinetis_internal_serial_endpoint_peripheral(endpoint);
...
static void queue_event(Kinetis* device, const K22IoEvent* incoming_event) {
...
bool kinetis_internal_peripheral_clock_enabled(const Kinetis* device, K22PeripheralId id) {
    const uint32_t scgc1 = kinetis_internal_raw_load(device, K22_SIM_SCGC1, 4);
    ...
    case K22_PERIPHERAL_DAC0:
        return device->profile->id >= K22_PROFILE_MK22FN1M012
                   ? gate(kinetis_internal_raw_load(device, K22_SIM_SCGC2, 4), 12)
                   : gate(scgc6, 31);
    case K22_PERIPHERAL_DAC1:
        return device->profile->id >= K22_PROFILE_MK22FN1M012
                   ? gate(kinetis_internal_raw_load(device, K22_SIM_SCGC2, 4), 13)
                   : gate(scgc6, 8);
