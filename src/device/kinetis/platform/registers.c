...
bool kinetis_peripheral_read(Kinetis* device, uint32_t address, uint8_t access_size,
    ...
    if (location.id != K22_PERIPHERAL_MCM && !kinetis_internal_manifest_extension(location.id) &&
...
void kinetis_peripheral_advance(Kinetis* device, uint32_t cycle_count) {
    ...
    if (k22_profile_has_peripheral(device->profile, K22_PERIPHERAL_CMT) &&
        kinetis_internal_peripheral_clock_enabled(device, K22_PERIPHERAL_CMT)) {
        kinetis_internal_cmt_advance(device, cycle_count);
    }
    if (k22_profile_has_peripheral(device->profile, K22_PERIPHERAL_USBDCD) &&
        kinetis_internal_peripheral_clock_enabled(device, K22_PERIPHERAL_USBDCD)) {
