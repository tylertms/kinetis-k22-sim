...
bool kinetis_set_ftm_quadrature_input(Kinetis* device, uint8_t instance, uint8_t phase,
                                      bool input_high) {
    if (device == NULL)
        return false;
    return kinetis_timing_set_ftm_quadrature_input(&device->timing, instance, phase, input_high);
}
