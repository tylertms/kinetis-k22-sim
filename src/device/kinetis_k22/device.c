...
static bool sysmpu_access_allowed(KinetisK22* device, uint32_t address, uint8_t access_size,
    ...
    if (!k22_io_sysmpu_access(&device->io, address, master, supervisor, access))
        return false;
    return k22_io_sysmpu_access(&device->io, address + access_size - 1u, master, supervisor,
                                access);
}

static bool memory_read_unprotected(KinetisK22* device, uint32_t address, uint8_t access_size,
                                    CortexM4Access access, uint8_t flash_master_id,
                                    uint32_t* output_value) {
    if (valid_range(address, access_size, K22_FLASH_BASE, device->configuration.flash_size)) {
        if (!flash_access_allowed(device, access, flash_master_id, false)) {
            return false;
        }
        if (!k22_data_flash_read(device->data, false, address, access_size)) {
    ...
    if (valid_range(address, access_size, device->sram_base, device->configuration.sram_size)) {
        *output_value = load_little_endian(device->sram + address - device->sram_base, access_size);
        return true;
    }
    if (device->profile->flexnvm_size != 0u &&
        valid_range(address, access_size, device->profile->flexnvm_address,
                    device->profile->flexnvm_size)) {
        if (!flash_access_allowed(device, access, flash_master_id, false))
            return false;
        const uint32_t offset = address - device->profile->flexnvm_address;
        if (!k22_data_flash_read(device->data, true, offset, access_size)) {
            *output_value = access_size == 4u ? UINT32_MAX : (1u << (access_size * 8u)) - 1u;
            return true;
        }
        ...
        return k22_data_read(device->data, address, access_size, output_value);
    }
    if (device->profile->flexram_size != 0u &&
        valid_range(address, access_size, device->profile->flexram_address,
    ...
    if (device->flexbus_memory != NULL &&
        valid_range(address, access_size, device->flexbus_address, device->flexbus_size) &&
        k22_io_flexbus_transfer(&device->io, address, access_size, false, 0u)) {
        *output_value = load_little_endian(
            device->flexbus_memory + address - device->flexbus_address, access_size);
        return true;
    }
    return kinetis_k22_peripheral_read(device, address, access_size, access, output_value);
}

static bool memory_write_unprotected(KinetisK22* device, uint32_t address, uint8_t access_size,
                                     CortexM4Access access, uint8_t flash_master_id,
    ...
    if (valid_range(address, access_size, device->sram_base, device->configuration.sram_size)) {
        store_little_endian(device->sram + address - device->sram_base, access_size, write_value);
        return true;
    }
    if (device->profile->flexnvm_size != 0u &&
        valid_range(address, access_size, device->profile->flexnvm_address,
                    device->profile->flexnvm_size)) {
        if (access != CORTEX_M4_ACCESS_DEBUG ||
            !k22_data_write(device->data, address, access_size, write_value))
            return false;
        const uint32_t offset = address - device->profile->flexnvm_address;
    ...
    if (device->profile->flexram_size != 0u &&
        valid_range(address, access_size, device->profile->flexram_address,
                    device->profile->flexram_size)) {
        if (!flash_access_allowed(device, access, flash_master_id, true))
            return false;
        return k22_data_write(device->data, address, access_size, write_value);
    }
    if (device->flexbus_memory != NULL && !device->flexbus_read_only &&
        valid_range(address, access_size, device->flexbus_address, device->flexbus_size) &&
        k22_io_flexbus_transfer(&device->io, address, access_size, true, write_value)) {
        store_little_endian(device->flexbus_memory + address - device->flexbus_address, access_size,
                            write_value);
        return true;
    }
    return kinetis_k22_peripheral_write(device, address, access_size, access, write_value);
}
...
bool kinetis_k22_memory_write(KinetisK22* device, uint32_t address, uint8_t access_size,
    ...
    if (access != CORTEX_M4_ACCESS_DEBUG) {
        const bool supervisor = access != CORTEX_M4_ACCESS_UNPRIVILEGED_DATA;
        if (!sysmpu_access_allowed(device, address, access_size, 1u, supervisor, K22_SYSMPU_WRITE))
            return false;
    }
...
bool kinetis_k22_dma_read(KinetisK22* device, uint32_t address, uint8_t access_size,
    ...
    if (device == NULL || output_value == NULL ||
        (access_size != 1u && access_size != 2u && access_size != 4u) ||
        !sysmpu_access_allowed(device, address, access_size, 2u, true, K22_SYSMPU_READ))
        return false;
...
bool kinetis_k22_dma_write(KinetisK22* device, uint32_t address, uint8_t access_size,
                           uint32_t write_value) {
    if (device == NULL || (access_size != 1u && access_size != 2u && access_size != 4u) ||
        !sysmpu_access_allowed(device, address, access_size, 2u, true, K22_SYSMPU_WRITE))
        return false;
...
bool kinetis_k22_flash_controller_write(KinetisK22* device, uint32_t address, uint8_t access_size,
                                        uint32_t write_value) {
    if (device == NULL || (access_size != 1u && access_size != 2u && access_size != 4u) ||
        !valid_range(address, access_size, K22_FLASH_BASE, device->configuration.flash_size))
        return false;
    store_little_endian(device->flash + address, access_size, write_value);
    return true;
}
...
static bool bit_band_read(KinetisK22* device, uint32_t address, CortexM4Access access,
    ...
    const uint32_t register_byte_address = K22_PERIPHERAL_BASE + alias_address / 32u;
    const uint8_t bit_index = (uint8_t)((alias_address / 4u) & 7u);
    const K22RegisterDescriptor* descriptor = bit_band_descriptor(device, register_byte_address);
...
static bool k22_read_bus(void* context, uint32_t address, uint8_t access_size,
                         CortexM4Access access, uint32_t* output_value) {
    ...
    return kinetis_k22_memory_read(device, address, access_size, access, output_value);
}

static bool k22_write_bus(void* context, uint32_t address, uint8_t access_size,
                          CortexM4Access access, uint32_t write_value) {
    KinetisK22* device = context;
    if (address >= K22_BIT_BAND_BASE && address < K22_BIT_BAND_BASE + K22_BIT_BAND_SIZE) {
        if (access_size != 1u && access_size != 2u && access_size != 4u)
...
static void destroy_partial_device(KinetisK22* device) {
    if (device == NULL) {
        return;
    }
...
void kinetis_k22_destroy(KinetisK22* device) { destroy_partial_device(device); }
...
void kinetis_k22_warm_reset(KinetisK22* device, uint8_t reset_cause_0, uint8_t reset_cause_1) {
    ...
    k22_timing_warm_reset(&device->timing, reset_cause_0, reset_cause_1);
    kinetis_k22_sync_clock_gates(device);
...
bool kinetis_k22_seed(KinetisK22* device, uint32_t address, const void* input_data,
                      size_t data_size) {
    return kinetis_k22_write(device, address, input_data, data_size);
}
...
bool kinetis_k22_read(const KinetisK22* device, uint32_t address, void* output_data,
    ...
    while (byte_offset < data_size) {
        ...
        if (!k22_read_bus((KinetisK22*)device, address + (uint32_t)byte_offset, access_size,
                          CORTEX_M4_ACCESS_DEBUG, &transfer_value)) {
            return false;
        }
        store_little_endian(output_bytes + byte_offset, access_size, transfer_value);
    ...
    while (byte_offset < data_size) {
        ...
        const uint8_t access_size = remaining_bytes >= 4 && ((address + byte_offset) & 3u) == 0 ? 4
                                    : remaining_bytes >= 2 && ((address + byte_offset) & 1u) == 0
                                        ? 2
                                        : 1;
        const uint32_t transfer_value = load_little_endian(input_bytes + byte_offset, access_size);
        if (!k22_write_bus(device, address + (uint32_t)byte_offset, access_size,
                           CORTEX_M4_ACCESS_DEBUG, transfer_value)) {
            return false;
        }
        byte_offset += access_size;
...
void kinetis_k22_advance(KinetisK22* device, uint32_t cycle_count) {
    if (device != NULL && cycle_count != 0u) {
        k22_advance_bus(device, cycle_count);
    }
}
