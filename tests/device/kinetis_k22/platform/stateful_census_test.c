...
static void exercise_signal(KinetisK22* device, StatefulCensus* census, uint32_t random_value) {
    bool operation_succeeded = false;
    bool ftm_output_high = false;
    uint16_t dac_output = 0u;
    switch (random_value % 16u) {
    case 0u:
        operation_succeeded = kinetis_k22_set_adc_channel(
            device, (uint8_t)(random_value >> 8u) % 3u, (uint8_t)(random_value >> 12u) % 32u,
            (uint16_t)random_value);
        break;
    case 1u:
        operation_succeeded =
            kinetis_k22_set_cmp_input(device, (uint8_t)(random_value >> 8u) % 4u,
                                      (uint8_t)(random_value >> 12u) % 9u, (uint8_t)random_value);
        break;
    case 2u:
        operation_succeeded = kinetis_k22_set_lptmr_input(
            device, (uint8_t)(random_value >> 8u) % 3u, (random_value & 1u) != 0u);
        break;
    case 3u:
        operation_succeeded = kinetis_k22_set_llwu_pin(device, (uint8_t)(random_value >> 8u) % 18u,
                                                       (random_value & 1u) != 0u);
        break;
    case 4u:
        operation_succeeded =
            kinetis_k22_trigger_llwu_module(device, (uint8_t)(random_value >> 8u) % 10u);
        break;
    case 5u:
        operation_succeeded = kinetis_k22_set_ftm_input(device, (uint8_t)(random_value >> 8u) % 6u,
                                                        (uint8_t)(random_value >> 12u) % 10u,
                                                        (random_value & 1u) != 0u);
        break;
    case 6u:
        operation_succeeded = kinetis_k22_set_ftm_fault(device, (uint8_t)(random_value >> 8u) % 6u,
                                                        (uint8_t)(random_value >> 12u) % 6u,
                                                        (random_value & 1u) != 0u);
        break;
    case 7u:
        operation_succeeded = kinetis_k22_trigger_ftm_hardware(
            device, (uint8_t)(random_value >> 8u) % 6u, (uint8_t)(random_value >> 12u) % 5u);
        break;
    case 8u:
        operation_succeeded =
            kinetis_k22_get_ftm_output(device, (uint8_t)(random_value >> 8u) % 6u,
                                       (uint8_t)(random_value >> 12u) % 10u, &ftm_output_high);
        mix_fingerprint(census, ftm_output_high);
        break;
    case 9u:
        operation_succeeded =
            kinetis_k22_get_dac_output(device, (uint8_t)(random_value >> 8u) % 3u, &dac_output);
        mix_fingerprint(census, dac_output);
        break;
    case 10u:
        operation_succeeded =
            kinetis_k22_gpio_drive(device, (uint8_t)(random_value >> 8u) % 7u,
                                   (uint8_t)(random_value >> 12u) % 34u, (random_value & 1u) != 0u);
        break;
    case 11u:
        operation_succeeded = kinetis_k22_gpio_release(device, (uint8_t)(random_value >> 8u) % 7u,
                                                       (uint8_t)(random_value >> 12u) % 34u);
        break;
    case 12u:
        operation_succeeded =
            kinetis_k22_serial_receive(device, (KinetisK22SerialEndpoint)(random_value % 15u),
                                       (uint16_t)random_value, (uint8_t)(random_value >> 16u));
        break;
    case 13u:
        operation_succeeded =
            kinetis_k22_set_usb_charger(device, (KinetisK22UsbCharger)(random_value % 6u));
        break;
    case 14u:
        operation_succeeded = kinetis_k22_set_usb_pullup(device, (random_value & 1u) != 0u);
        break;
    default:
        operation_succeeded = kinetis_k22_set_ewm_input(device, (random_value & 1u) != 0u);
        break;
    }
...
static void exercise_profile(TestState* state, StatefulCensus* census, KinetisK22Profile profile) {
    ...
    for (uint32_t iteration = 0u; iteration < 30000u; iteration++) {
        ...
        const bool write_succeeded = kinetis_k22_memory_write(
            device, address, access_size, access_type, random_value ^ UINT32_C(0xa5a55a5a));
        uint32_t read_value = UINT32_MAX;
        const bool read_succeeded =
            kinetis_k22_memory_read(device, address, access_size, access_type, &read_value);
        ...
        if ((iteration & 15u) == 0u) {
            kinetis_k22_advance(device, (random_value & 31u) + 1u);
        }
        KinetisK22Event event;
        for (uint8_t event_index = 0u; event_index < 4u && kinetis_k22_next_event(device, &event);
             event_index++) {
...
int main(void) {
    ...
    const bool census_matches = census.read_successes == 35164u &&
                                ...
                                census.fingerprint == UINT64_C(16146922478742010809);
    if (!census_matches) {
        fprintf(stderr,
