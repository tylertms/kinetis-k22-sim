...
static void write_debug(TestState* state, KinetisK22* device, uint32_t register_address,
                        const void* write_data, size_t byte_count) {
    expect(state, kinetis_k22_write(device, register_address, write_data, byte_count),
           "kinetis_k22_write(device, register_address, write_data, byte_count)");
}

static uint32_t read_debug(TestState* state, KinetisK22* device, uint32_t register_address) {
...
static void configure_transfer(TestState* state, KinetisK22* device, uint32_t source,
...
int main(void) {
    ...
    write_debug(&state, device, SYSMPU_CESR, &(uint32_t){(1u << 27u) | 1u}, sizeof(uint32_t));
    expect(&state,
           kinetis_k22_memory_read(device, source, 1u, CORTEX_M4_ACCESS_DATA, &memory_read_value),
           "kinetis_k22_memory_read(device, source, 1u, CORTEX_M4_ACCESS_DATA, "
           "&memory_read_value)");
    ...
    expect(&state,
           kinetis_k22_read(device, destination, &destination_byte, sizeof(destination_byte)),
           "kinetis_k22_read(device, destination, &destination_byte, sizeof(destination_byte))");
