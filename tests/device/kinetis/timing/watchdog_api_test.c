...
static KinetisTiming make_timing(TestState* state, Observations* observations) {
    ...
    expect(
        state,
        kinetis_timing_init(&timing, kinetis_profile_get(KINETIS_PROFILE_MK22FN51212), 8000000u,
                            32768u, signals),
        "kinetis_timing_init(&timing, kinetis_profile_get(KINETIS_PROFILE_MK22FN51212), 8000000u, "
        "32768u, signals)");
    return timing;
...
static void write_byte(TestState* state, KinetisTiming* timing, uint32_t address, uint8_t value) {
    expect(state, kinetis_timing_write(timing, address, 1u, value),
           "kinetis_timing_write(timing, address, 1u, value)");
}

static uint8_t read_byte(TestState* state, KinetisTiming* timing, uint32_t address) {
    uint32_t value = UINT32_MAX;
    expect(state, kinetis_timing_read(timing, address, 1u, &value),
           "kinetis_timing_read(timing, address, 1u, &value)");
    return (uint8_t)value;
}

static uint32_t core_cycles_for_bus_cycles(const KinetisTiming* timing, uint32_t bus_cycles) {
    return (uint32_t)(((uint64_t)bus_cycles * timing->core_clock_hz + timing->bus_clock_hz - 1u) /
                      timing->bus_clock_hz);
}

static uint32_t core_cycles_for_lpo_cycles(const KinetisTiming* timing, uint32_t lpo_cycles) {
    return (uint32_t)(((uint64_t)lpo_cycles * timing->core_clock_hz + timing->lpo_hz - 1u) /
                      timing->lpo_hz);
}

static void advance_bus(KinetisTiming* timing, uint32_t bus_cycles) {
    kinetis_timing_advance(timing, core_cycles_for_bus_cycles(timing, bus_cycles));
}

static void unlock(TestState* state, KinetisTiming* timing) {
    write_register(state, timing, WDOG_UNLOCK, 0xc520u);
    write_register(state, timing, WDOG_UNLOCK, 0xd928u);
}

static void configure(TestState* state, KinetisTiming* timing, uint32_t timeout, uint32_t window,
                      uint16_t control) {
    unlock(state, timing);
...
static void disable_watchdog(KinetisTiming* timing) {
    timing->wdog[0] = 0u;
...
static void test_configuration_windows(TestState* state) {
    ...
    KinetisTiming timing = make_timing(state, &observations);
...
static void test_refresh_and_interrupt(TestState* state) {
    ...
    kinetis_timing_watchdog_advance(&timing, 20u);
    ...
    expect(state, observations.resets == 0u, "observations.resets == 0u");
    kinetis_timing_watchdog_advance(&timing, 19u);
    write_register(state, &timing, WDOG_REFRESH, 0xa602u);
    write_register(state, &timing, WDOG_REFRESH, 0xb480u);
    expect(state, observations.resets == 1u, "observations.resets == 1u");
...
static void test_pause_and_byte_modes(TestState* state) {
    ...
    kinetis_timing_watchdog_advance(&timing, 2u);
    expect(state, observations.resets == 0u, "observations.resets == 0u");
    ...
    kinetis_timing_watchdog_advance(&timing, 0x43u);
    expect(state, observations.resets == 0u, "observations.resets == 0u");
    kinetis_timing_watchdog_advance(&timing, 1u);
    expect(state, observations.resets == 1u, "observations.resets == 1u");
}
...
static void test_byte_access_and_control(TestState* state) {
    ...
    kinetis_timing_watchdog_advance(&timing, 0x123u);
    ...
    expect(state, read_register(state, &timing, WDOG_RSTCNT) == 0u,
           "read_register(state, &timing, WDOG_RSTCNT) == 0u");
    kinetis_timing_watchdog_advance(&timing, 0x44u);
    expect(state, !observations.irq, "!observations.irq");
...
static void configure_ewm(TestState* state, KinetisTiming* timing, uint8_t low, uint8_t high,
                          uint8_t control) {
...
static void service_ewm(TestState* state, KinetisTiming* timing) {
...
static void test_external_watchdog(TestState* state) {
    ...
    expect(state, kinetis_timing_set_ewm_input(&timing, true),
           "kinetis_timing_set_ewm_input(&timing, true)");
    configure_ewm(state, &timing, 1u, 4u, 0x0du);
    expect(state, !kinetis_timing_ewm_output(&timing), "!kinetis_timing_ewm_output(&timing)");
    expect(state, !kinetis_timing_write(&timing, EWM_CMPL, 1u, 2u),
           "!kinetis_timing_write(&timing, EWM_CMPL, 1u, 2u)");
    kinetis_timing_advance(&timing, core_cycles_for_lpo_cycles(&timing, 2u));
    ...
    expect(state, !kinetis_timing_ewm_output(&timing), "!kinetis_timing_ewm_output(&timing)");
    kinetis_timing_advance(&timing, core_cycles_for_lpo_cycles(&timing, 4u));
    expect(state, kinetis_timing_ewm_output(&timing), "kinetis_timing_ewm_output(&timing)");
    expect(state, observations.irq, "observations.irq");
    ...
    expect(state, kinetis_timing_set_ewm_input(&timing, true),
           "kinetis_timing_set_ewm_input(&timing, true)");
    configure_ewm(state, &timing, 1u, 4u, 0x0du);
    kinetis_timing_advance(&timing, core_cycles_for_lpo_cycles(&timing, 1u));
    service_ewm(state, &timing);
    expect(state, kinetis_timing_ewm_output(&timing), "kinetis_timing_ewm_output(&timing)");
    ...
    kinetis_timing_advance(&timing, core_cycles_for_lpo_cycles(&timing, 4u));
    expect(state, kinetis_timing_ewm_output(&timing), "kinetis_timing_ewm_output(&timing)");
    expect(state, !kinetis_timing_set_ewm_input(NULL, false),
           "!kinetis_timing_set_ewm_input(NULL, false)");
    expect(state, !kinetis_timing_ewm_output(NULL), "!kinetis_timing_ewm_output(NULL)");
}
