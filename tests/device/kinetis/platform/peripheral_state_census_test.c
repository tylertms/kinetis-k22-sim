...
int main(void) {
    ...
    Kinetis* device = kinetis_create(kinetis_configuration(KINETIS_PROFILE_MK22FN1M012));
    ...
    for (uint32_t iteration = 0u; iteration < 100000u; iteration++) {
        ...
        const bool write_succeeded =
            kinetis_internal_semantic_write(device, location.id, descriptor->address, access_size,
                                            random_value ^ UINT32_C(0xa5a55a5a));
        const bool serial_endpoint_available = kinetis_internal_serial_endpoint_available(
            device,
            (KinetisSerialEndpoint)((random_value >> 12u) % (KINETIS_SERIAL_ENDPOINT_COUNT + 2u)));
        kinetis_internal_cmt_advance(device, (random_value & 31u) + 1u);
        if ((iteration & 15u) == 0u) {
    ...
    const bool census_matches = census.accepted_accesses == 411511u &&
                                census.rejected_accesses == 288489u &&
                                census.fingerprint == UINT64_C(18429700810583176930);
    if (!census_matches) {
        fprintf(stderr,
