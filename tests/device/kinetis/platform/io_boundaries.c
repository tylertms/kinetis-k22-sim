...
static void expect_read(TestState* state, K22Io* io, uint32_t address, uint8_t access_size) {
    uint32_t read_value = 0u;
    expect(state, k22_io_read(io, address, access_size, &read_value),
           "peripheral boundary read succeeds");
}

static void expect_rejected_read(TestState* state, K22Io* io, uint32_t address,
                                 uint8_t access_size) {
    uint32_t read_value = 0u;
    expect(state, !k22_io_read(io, address, access_size, &read_value),
...
static void test_can_offsets(TestState* state, K22Io* io) {
    ...
    for (size_t offset_index = 0u;
        ...
        expect(state, !k22_io_write(io, CAN0 + read_only_offsets[offset_index], 4u, 0u),
...
static void test_i2s_offsets(TestState* state, K22Io* io) {
    ...
    for (size_t offset_index = 0u;
         offset_index < sizeof(read_only_offsets) / sizeof(read_only_offsets[0]); offset_index++) {
        expect(state, !k22_io_write(io, I2S0 + read_only_offsets[offset_index], 4u, 0u),
...
void k22_io_test_peripheral_boundaries(TestState* state) {
    ...
    expect(state, k22_io_init(&usb_io, usb_configuration), "USB boundary fixture initializes");
    k22_io_set_clock(&usb_io, K22_PERIPHERAL_USB0, true);
    ...
    expect(state, k22_io_init(&bus_io, bus_configuration), "bus boundary fixture initializes");
    k22_io_set_clock(&bus_io, K22_PERIPHERAL_CAN0, true);
    k22_io_set_clock(&bus_io, K22_PERIPHERAL_I2S0, true);
    k22_io_set_clock(&bus_io, K22_PERIPHERAL_FB, true);
    k22_io_set_clock(&bus_io, K22_PERIPHERAL_SYSMPU, true);
