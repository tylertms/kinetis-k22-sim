...
static uint32_t read32(TestState* test_state, Kinetis* device, uint32_t register_address) {
    uint32_t output_value = UINT32_MAX;
    expect(test_state, kinetis_read(device, register_address, &output_value, sizeof(output_value)),
           "kinetis_read(device, register_address, &output_value, sizeof(output_value))");
    return output_value;
}

static void write32(TestState* state, Kinetis* device, uint32_t register_address,
                    uint32_t write_value) {
    expect(state, kinetis_write(device, register_address, &write_value, sizeof(write_value)),
           "kinetis_write(device, register_address, &write_value, sizeof(write_value))");
}

static uint8_t read8(TestState* test_state, Kinetis* device, uint32_t register_address) {
    uint8_t output_value = UINT8_MAX;
    expect(test_state, kinetis_read(device, register_address, &output_value, sizeof(output_value)),
           "kinetis_read(device, register_address, &output_value, sizeof(output_value))");
    return output_value;
}

static uint16_t read16(TestState* test_state, Kinetis* device, uint32_t register_address) {
    uint16_t output_value = UINT16_MAX;
    expect(test_state, kinetis_read(device, register_address, &output_value, sizeof(output_value)),
           "kinetis_read(device, register_address, &output_value, sizeof(output_value))");
    return output_value;
}

static void write8(TestState* state, Kinetis* device, uint32_t register_address,
                   uint8_t write_value) {
    expect(state, kinetis_write(device, register_address, &write_value, sizeof(write_value)),
           "kinetis_write(device, register_address, &write_value, sizeof(write_value))");
}

static void write16(TestState* state, Kinetis* device, uint32_t register_address,
                    uint16_t write_value) {
    expect(state, kinetis_write(device, register_address, &write_value, sizeof(write_value)),
           "kinetis_write(device, register_address, &write_value, sizeof(write_value))");
}

static void configure_cmt_dma(TestState* state, Kinetis* device) {
...
static void test_cmt(TestState* state, Kinetis* device) {
    ...
    expect(state, !cortex_m4_get_irq_pending(kinetis_cpu(device), 45u),
           "!cortex_m4_get_irq_pending(kinetis_cpu(device), 45u)");
    kinetis_advance(device, 7u);
    expect(state, (read8(state, device, CMT_MSC) & 0x80u) == 0u,
           "(read8(state, device, CMT_MSC) & 0x80u) == 0u");
    kinetis_advance(device, 8u);
    expect(state, (read8(state, device, CMT_MSC) & 0x80u) == 0u,
           "(read8(state, device, CMT_MSC) & 0x80u) == 0u");
    ...
    expect(state, device->cmt_cycles != 0u, "device->cmt_cycles != 0u");
    kinetis_advance(device, 14u);
    expect(state, device->cmt_cycles == 0u, "device->cmt_cycles == 0u");
    expect(state, !device->cmt_running, "!device->cmt_running");
}

static void test_event_capacity(TestState* state, Kinetis* device) {
    KinetisEvent pending_event;
...
static void test_timing_dma(TestState* state, Kinetis* device) {
...
static void test_usbdcd(TestState* state, Kinetis* device) {
    write32(state, device, SIM_SCGC6, read32(state, device, SIM_SCGC6) | (1u << 21u));
    expect(state, kinetis_set_usb_charger(device, KINETIS_USB_CHARGER_STANDARD_HOST),
           "kinetis_set_usb_charger(device, KINETIS_USB_CHARGER_STANDARD_HOST)");
    ...
    expect(state, cortex_m4_get_irq_pending(kinetis_cpu(device), 54u),
           "cortex_m4_get_irq_pending(kinetis_cpu(device), 54u)");
    write32(state, device, USBDCD_CONTROL, 1u);
    expect(state, (read32(state, device, USBDCD_CONTROL) & (1u << 8u)) == 0u,
           "(read32(state, device, USBDCD_CONTROL) & (1u << 8u)) == 0u");
    write32(state, device, USBDCD_CONTROL, (1u << 16u) | (1u << 25u));
    expect(state, kinetis_set_usb_charger(device, KINETIS_USB_CHARGER_CHARGING_PORT),
           "kinetis_set_usb_charger(device, KINETIS_USB_CHARGER_CHARGING_PORT)");
    ...
    expect(state, kinetis_set_usb_charger(device, KINETIS_USB_CHARGER_DEDICATED),
           "kinetis_set_usb_charger(device, KINETIS_USB_CHARGER_DEDICATED)");
    write32(state, device, USBDCD_CONTROL, (1u << 17u) | (1u << 16u) | (1u << 24u));
    kinetis_advance(device, 5u);
    ...
    expect(state,
           !kinetis_set_usb_charger(device, (KinetisUsbCharger)(KINETIS_USB_CHARGER_ERROR + 1)),
           "!kinetis_set_usb_charger( device, "
           "(KinetisUsbCharger)(KINETIS_USB_CHARGER_ERROR + 1))");
}

static void test_access_controls(TestState* state, Kinetis* device) {
    write32(state, device, AIPS0_PACRI, 1u << 20u);
    uint32_t access_read_value = 0u;
    expect(state,
           kinetis_peripheral_read(device, CMT_MSC, 1u, CORTEX_M4_ACCESS_UNPRIVILEGED_DATA,
                                   &access_read_value),
           "kinetis_peripheral_read(device, CMT_MSC, 1u, "
    ...
    expect(state,
           ...
           "!kinetis_peripheral_read(device, CMT_MSC, 1u, "
           "CORTEX_M4_ACCESS_UNPRIVILEGED_DATA, &access_read_value)");
    cortex_m4_set_control(kinetis_cpu(device), CORTEX_M4_CONTROL_NPRIV);
    expect(state,
           !kinetis_peripheral_read(device, CMT_MSC, 1u, CORTEX_M4_ACCESS_DATA, &access_read_value),
           "!kinetis_peripheral_read(device, CMT_MSC, 1u, CORTEX_M4_ACCESS_DATA, "
           "&access_read_value)");
    cortex_m4_set_control(kinetis_cpu(device), 0u);
    expect(state,
           kinetis_peripheral_read(device, CMT_MSC, 1u, CORTEX_M4_ACCESS_DATA, &access_read_value),
           "kinetis_peripheral_read(device, CMT_MSC, 1u, CORTEX_M4_ACCESS_DATA, "
           "&access_read_value)");
    expect(state, !kinetis_peripheral_write(device, CMT_MSC, 1u, CORTEX_M4_ACCESS_DATA, 0u),
           "!kinetis_peripheral_write(device, CMT_MSC, 1u, CORTEX_M4_ACCESS_DATA, 0u)");
    write32(state, device, AIPS0_PACRI, 0u);

    write32(state, device, AXBS_CRS0, 0x80000000u);
    uint32_t priority = 0x12345678u;
    expect(state, !kinetis_write(device, AXBS_PRS0, &priority, sizeof(priority)),
           "!kinetis_write(device, AXBS_PRS0, &priority, sizeof(priority))");

    write32(state, device, FMC_PFAPR, 0u);
    expect(
        state,
        !kinetis_memory_read(device, 0x100u, 2u, CORTEX_M4_ACCESS_INSTRUCTION, &access_read_value),
        "!kinetis_memory_read(device, 0x100u, 2u, CORTEX_M4_ACCESS_INSTRUCTION, "
        "&access_read_value)");
    expect(state,
           kinetis_memory_read(device, 0x100u, 2u, CORTEX_M4_ACCESS_DEBUG, &access_read_value),
           "kinetis_memory_read(device, 0x100u, 2u, CORTEX_M4_ACCESS_DEBUG, "
           "&access_read_value)");
}

static void test_fmc_geometry(TestState* state, Kinetis* device) {
