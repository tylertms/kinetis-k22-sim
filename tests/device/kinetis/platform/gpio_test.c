...
static uint32_t read32(TestState* state, Kinetis* device, uint32_t register_address) {
...
int main(void) {
    ...
    for (uint8_t pin_index = 0u; pin_index < 32u; pin_index++) {
        if (pin_index != 3u) {
            kinetis_gpio_drive(device, 0u, pin_index, false);
        }
    }
    ...
    kinetis_gpio_drive(device, 0, 3, false);
    kinetis_gpio_drive(device, 0, 3, true);
    ...
    cortex_m4_set_irq(kinetis_cpu(device), PORTA_IRQ, false);
    expect(&state, (read32(&state, device, PORTA_PCR3) & (1u << 24)) != 0,
           "(read32(&state, device, PORTA_PCR3) & (1u << 24)) != 0");
    expect(&state, cortex_m4_get_irq_pending(kinetis_cpu(device), PORTA_IRQ),
           "cortex_m4_get_irq_pending(kinetis_cpu(device), PORTA_IRQ)");
