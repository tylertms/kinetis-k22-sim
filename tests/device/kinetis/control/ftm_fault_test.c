...
static void test_manual_fault_lifecycle(TestState* state) {
    ...
    expect(state, kinetis_timing_set_ftm_fault(&timing, 0u, 0u, true),
           "kinetis_timing_set_ftm_fault(&timing, 0u, 0u, true)");
    ...
    expect(state, kinetis_timing_set_ftm_fault(&timing, 0u, 0u, false),
           "kinetis_timing_set_ftm_fault(&timing, 0u, 0u, false)");
...
static void test_fault_inputs_and_filter(TestState* state) {
    for (uint8_t input = 0u; input < 4u; input++) {
        ...
        expect(state, kinetis_timing_set_ftm_fault(&timing, 0u, input, true),
               "kinetis_timing_set_ftm_fault(&timing, 0u, input, true)");
        kinetis_timing_advance(&timing, 3u);
    ...
    expect(state, kinetis_timing_set_ftm_fault(&timing, 0u, 0u, true),
           "kinetis_timing_set_ftm_fault(&timing, 0u, 0u, true)");
    kinetis_timing_advance(&timing, 5u);
    expect(state, read_register(state, &timing, FTM0_FMS) == 0u,
           "read_register(state, &timing, FTM0_FMS) == 0u");
    kinetis_timing_advance(&timing, 1u);
    expect(state, read_register(state, &timing, FTM0_FMS) == 0xa1u,
...
static void test_flag_clearing_and_irq_control(TestState* state) {
    ...
    for (uint8_t input = 0u; input < 4u; input++)
        expect(state, kinetis_timing_set_ftm_fault(&timing, 0u, input, false),
               "kinetis_timing_set_ftm_fault(&timing, 0u, input, false)");
    kinetis_timing_advance(&timing, 3u);
...
static void test_invalid_fault_inputs(TestState* state) {
    ...
    IrqRecorder recorder = {0};
    initialize(state, &timing, &recorder, 0x40u, 0x40u, 1u);
    expect(state, !kinetis_timing_set_ftm_fault(NULL, 0u, 0u, true),
           "!kinetis_timing_set_ftm_fault(NULL, 0u, 0u, true)");
    expect(state, !kinetis_timing_set_ftm_fault(&timing, 4u, 0u, true),
           "!kinetis_timing_set_ftm_fault(&timing, 4u, 0u, true)");
    expect(state, !kinetis_timing_set_ftm_fault(&timing, 0u, 4u, true),
           "!kinetis_timing_set_ftm_fault(&timing, 0u, 4u, true)");
}
