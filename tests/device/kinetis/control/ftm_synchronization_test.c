...
static void expect_register(TestState* state, KinetisTiming* timing, uint32_t address,
                            uint32_t expected) {
    uint32_t value = 0u;
...
static void initialize(TestState* state, KinetisTiming* timing) {
    const KinetisDeviceProfile* profile = kinetis_profile_get(KINETIS_PROFILE_MK22FN51212);
    expect(state, kinetis_timing_init(timing, profile, 8000000u, 32768u, (KinetisTimingSignals){0}),
           "kinetis_timing_init(timing, profile, 8000000u, 32768u, (KinetisTimingSignals){0})");
    write_register(state, timing, SIM_SCGC6, timing->sim_scgc6 | (1u << 24u));
}

static void set_counter(TestState* state, KinetisTiming* timing, uint32_t value) {
    write_register(state, timing, FTM0_CNT, 0u);
    write_register(state, timing, FTM0_SC, 8u);
    kinetis_timing_advance(timing, value - 2u);
...
static void test_hardware_trigger_domain(TestState* state) {
    expect(state, !kinetis_timing_trigger_ftm_hardware(NULL, 0u, 0u),
           "!kinetis_timing_trigger_ftm_hardware(NULL, 0u, 0u)");
    KinetisTiming timing;
    initialize(state, &timing);
    expect(state, !kinetis_timing_trigger_ftm_hardware(&timing, 4u, 0u),
           "!kinetis_timing_trigger_ftm_hardware(&timing, 4u, 0u)");
    expect(state, !kinetis_timing_trigger_ftm_hardware(&timing, 0u, 3u),
           "!kinetis_timing_trigger_ftm_hardware(&timing, 0u, 3u)");
    write_register(state, &timing, FTM0_MODE, 5u);
    write_register(state, &timing, FTM0_CNTIN, 2u);
    write_register(state, &timing, FTM0_CNT, 0u);
    write_register(state, &timing, FTM0_SYNCONF, (1u << 16u) | (1u << 7u));
    set_counter(state, &timing, 9u);
    expect(state, kinetis_timing_trigger_ftm_hardware(&timing, 0u, 0u),
           "kinetis_timing_trigger_ftm_hardware(&timing, 0u, 0u)");
    ...
    for (uint8_t trigger = 0u; trigger < 3u; trigger++) {
        ...
        expect(state, kinetis_timing_trigger_ftm_hardware(&timing, 0u, trigger),
               "kinetis_timing_trigger_ftm_hardware(&timing, 0u, trigger)");
...
static void prepare_hardware_write_buffers(TestState* state, KinetisTiming* timing,
...
static void prepare_legacy_hardware_sync(TestState* state, KinetisTiming* timing, uint32_t mode,
                                         uint32_t sync) {
    initialize(state, timing);
